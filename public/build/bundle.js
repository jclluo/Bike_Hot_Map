...
var app = (function (path, fs, preprocess) {
    'use strict';
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			t1 = space();
    			input = element("input");
    			add_location(label, file, 205, 2, 5894);
    			attr_dev(input, "id", "slider");
    			attr_dev(input, "type", "range");
    			attr_dev(input, "min", "1");
    			attr_dev(input, "max", "1440");
    			add_location(input, file, 206, 2, 5926);
    			attr_dev(div, "class", "overlay");
    			add_location(div, file, 204, 4, 5870);
    			add_location(main, file, 203, 0, 5859);
    		},
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	map.on("load", () => {
    		...
    		map.addLayer({
    			...
    			type: "line",
    			source: "boston_route",
    			paint: { "line-color": "#BEE5B0", "line-width": 2 }
    		});
    	});
    	...
    	const colorScale = d3.scaleLinear().domain([0, 0.5, 1]).range(["red", "white", "green"]);

    	//function of drawing the circles
    	function create_station_markers(station_data) {
    		station_markers = marker_container.selectAll("circle").data(station_data).enter().append("circle").attr("r", 5).style("fill", function (d) {
    			return calculateColor(d["station_id"]);
    		}).attr("stroke", "#808080").attr("stroke-width", 1).attr("fill-opacity", 0.8).attr("name", function (d) {
    			return d["name"];
    		});
    	...
    	function calculateColor(station_id) {
    		...
    		let arrivalRatio = totalTraffic > 0
    		? arrivals[station_id] / totalTraffic
    		: 0;
